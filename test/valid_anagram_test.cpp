    ...
    (forfun::valid_anagram::generic::is_anagram<char>))
{
    SECTION("Positive")
    {
        auto [s, t]{GENERATE(table<std::string_view, std::string_view>({
            {""sv, ""sv},
            {"cat"sv, "cat"sv},
...
TEST_CASE("Valid Anagram (8-bit ordinal comparison)", "[valid_anagram]")
{
    ...
    SECTION("Positive")
    {
        auto [s, t]{GENERATE(table<std::u8string_view, std::u8string_view>({
            {u8""sv, u8""sv},
            {u8"cat"sv, u8"cat"sv},
...
TEST_CASE("Valid Anagram (32-bit ordinal comparison)", "[valid_anagram]")
{
    ...
    SECTION("Positive")
    {
        auto [s, t]{GENERATE(table<std::u32string_view, std::u32string_view>({
            {U""sv, U""sv},
            {U"cat"sv, U"cat"sv},
